...
step=0.001;
delta=0.00018;
n=116906; % block length-->[total VND=2*n]
...
Dvi=[1:1:23];   % 可以亂tune，下面想要用 linprog 來解問題，真正的變數是 bvi
...
Icu=0.5*(1-(1-Ie_CND).^2)+0.25*(1-(1-Ie_CND).^3)+0.25*(1-(1-Ie_CND).^4);
%     Icu=(1-(1-Ie_CND).^2);
    Ie_DET=(1-Icu)*Iuc_i+Icu*Iuc_ni;
...
xlim([0,1])
ylim([0,1])
ylabel("I_{out,VND},I_{in,CND}")
xlabel("I_{in,VND},I_{out,CND}")
legend("CND","VND")
